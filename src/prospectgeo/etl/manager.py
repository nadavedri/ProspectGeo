...
def _run_prospect_pipeline():
    try:
        ...
        for chunk_idx, chunk in enumerate(load_csv_file_in_chunks("prospects.csv"), start=1):
...
def run_etl_pipeline(name: str):
    ...
    if not pipeline_fn:
        logger.error("Unknown pipeline '%s'. Available pipelines: %s", name, list(PIPELINES.keys()))
        raise ValueError(
